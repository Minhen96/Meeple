	...
	const sections = $derived([
		{
			title: 'Account',
			items: [
				{ label: 'Edit Profile', href: '/settings/profile', icon: 'person' },
				{ label: 'Change Password', href: '/settings/password', icon: 'lock' },
				{ label: 'BGG Import', href: '/settings/bgg', icon: 'cloud_download' },
		...
		}] : []),
		{
			title: 'Notifications',
			items: [
				{ label: 'Notification Preferences', href: '/settings/notifications', icon: 'notifications' }
			]
		},
		{
			title: 'About',
...
<div class="space-y-6">
	{#each sections as section}
		<div>
			...
			<div class="bg-surface-container-lowest rounded-xl overflow-hidden shadow-[0_12px_32px_rgba(25,28,29,0.06)]">
				{#each section.items as item, i}
					{#if item.href}
						<a
							href={item.href}
							class="flex items-center gap-3 px-4 py-3.5 hover:bg-surface-container-low transition-colors {i > 0 ? 'border-t border-outline-variant/10' : ''} {item.danger ? 'text-error' : 'text-on-surface'}"
						>
							<span class="material-symbols-outlined text-[20px] {item.danger ? 'text-error' : 'text-on-surface-variant'}">{item.icon}</span>
